...
class MultiOutputWidget : public QDockWidget {
    ...
    public:
    MultiOutputWidget(QWidget *parent = 0) : QDockWidget(parent), reopenShown_(false)
    {
        ...
        QObject::connect(this, &QDockWidget::dockLocationChanged,
                         [this](Qt::DockWidgetArea area) { dockLocation_ = area; });
        ...
        layout_->addWidget(login_ = createLoginWidget(
                                   [this](const char *u, const char *p) { OnLoginClicked(u, p); },
                                   container_));
    ...
    void visibleToggled(bool visible)
    {
        ...
        if (visible == false && reopenShown_ == false &&
            !config_has_user_value(obs_frontend_get_global_config(), ConfigSection,
                                   "DockVisible")) {
            reopenShown_ = true;
            QMessageBox(QMessageBox::Icon::Information, obs_module_text("Notice.Title"),
                        obs_module_text("Notice.Reopen"), QMessageBox::StandardButton::Ok, this)
                    .exec();
        }
    }
    ...
    void OnLoginClicked(const char *username, const char *password)
    {
        login_->SetLoginEnabled(false);
        std::thread([this, un = std::string(username), pw = std::string(password)]() {
            auto res = ::Login(un, pw);
            RunInUIThread([this, res]() {
                LoginCallback(res);
                login_->SetLoginEnabled(true);
            });
        }).detach();
    }
    ...
    {
        if (result.empty()) {
            QMessageBox(QMessageBox::Icon::Critical, "Error", "Fail to call server.",
        ...
        Json converted;
        if (!LoadLoginResult(result, converted)) {
            QMessageBox(QMessageBox::Icon::Critical, "Error", "Login failed.",
                        QMessageBox::StandardButton::Ok, this)
                    .exec();
            return;
        }

        login_->setVisible(false);
        LoadConfig(converted);
    }

    void LoadConfig(Json conf)
    {
        for (auto x : conf["servers"].array_items()) {
            // auto pushwidget = createPushWidget(((QJsonValue)x).toObject(), container_);
            // layout_->addWidget(pushwidget);
        }
        // auto targets = conf.find("targets");
...
bool obs_module_load()
{
    ...
    if (mainwin == nullptr)
        return false;
    QMetaObject::invokeMethod(mainwin, []() { g_uiThread = QThread::currentThread(); });
